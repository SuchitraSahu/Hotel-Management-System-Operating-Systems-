...
int main(){
	...
	key_t key =ftok("earnings.txt",1); //creating file
	if (key == -1) {
	...
	shared_memory[0]=0; //specification
	
	
	
	
	
	FILE *file = fopen("earnings.txt", "w"); //writing access
	//Shared Memory creation
	int *SM[numberoftables]; //int ptr of size = no. of tables ,stores ptrs of diff shared mems
	for(int i=0;i<numberoftables;i++){
		key_t key1 =ftok("menu.txt",i+11); //0-10 was for waiter
		if (key1 == -1) {
        		printf("ftok falliure");
        		return -1;
    		}
